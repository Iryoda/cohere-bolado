...
export function calculateSearchParams(
  ...
  for (let i = 0; i < 22; i++) {
    ...
    if (value > 3.5) result += musicLabels[i] + "Very High";
    else if (value > 1) result += musicLabels[i] + "High";
    else if (value < -3.5) result += musicLabels[i] + "Very Low";
    else if (value < -1) result += musicLabels[i] + "Low";
    else result += musicLabels[i] + "Medium";
  }
